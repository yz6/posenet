<template>
    <div style="overflow: hidden;height: 100%;position: relative" @mouseup="allMouseup">
        <div class="rightTitle" style="margin: 25px 29px">
            ...
            <span class="title-words">使用中的工位</span>
            &nbsp; &nbsp; &nbsp;
        ...
        <div class="bottomActionBar">
            ...
            <div class="barBtn">
                <el-button :disabled="checkedSeatData.length==0" type="primary" @click="againReserve">再次预约</el-button>
                <el-button :disabled="checkedSeatData.length==0" type="primary" @click="logoutSeat">注销工位</el-button>
            </div>
        </div>
        <el-dialog
            title="注销工位"
            :visible.sync="logoutDiaShow"
            width="560px"
            center>
            <div class="diaTagBox">
                <div class="tagsBox">
                    <div class="diaTag" @click="clickLogoutSeat(ind)" :class="item.act?'act':''" v-for="(item,ind) in logoutSeatData">{{item.seatName}}</div>
                </div>
            </div>
            <span slot="footer" class="dialog-footer">
    <el-button @click="logoutDiaShow = false">取 消</el-button>
    <el-button type="primary" :disabled="logoutDis" @click="logoutSeatSub">确 定</el-button>
  </span>
        </el-dialog>
    </div>
</template>
...
<script>
    import {getMapSeatData,reservedDetail,cancelReservation,getFloorInfo,staffLogout} from "../../api/getData";
    ...
    export default {
        ...
        methods: {
            ...
            //再次预约
            againReserve(){
                this.$router.push({path:'/reReserve_seat',query:{id:this.reservedData.id}})
            },
            //注销工位
            logoutSeat(){
                this.logoutDiaShow = true
                this.logoutDis = true
                this.logoutSeatData = this.checkedSeatData
                this.logoutSeatData.forEach((v,i)=>{
                    this.$set(this.logoutSeatData[i],'act',false)
                })
            },
            //选择取消的工位
            clickLogoutSeat(i){
                this.$set(this.logoutSeatData[i],'act',!this.logoutSeatData[i].act)
                this.logoutDis = JSON.stringify(this.logoutSeatData).indexOf('"act":true') == -1
