<template>
    <div style="overflow: hidden;height: 100%;position: relative" @mouseup="allMouseup">
        ...
        <div class="legendBox">
            <div class="item"><img src="../../assets/img/legend01.png" alt="">不可选工位</div>
            <div class="item"><img src="../../assets/img/legend04.png" alt="">可选工位</div>
        ...
        <div class="fillcontain" id="fillcontain" ref="fillcontain" style="overflow: hidden;">
            ...
            <div class="outer" ref="outer" id="outer">
                ...
                <span @click="item.staffDepartment==department && item.type==1?checkSeat(item,index):''" v-for="(item,index) in seatData"
                     ...
                     /*判断管理员还是部门经理*/
                      ...
                      top:item.pointY*scale+'px',
                       backgroundImage:'url('+ (
                      item.toward==0?item.act?rightSeat_act:item.staffDepartment==department?item.status == 1?item.type==1?rightSeat_move:rightSeat_move:rightSeat_dis:rightSeat_dis:
                      item.toward==1?item.act?topSeat_act:item.staffDepartment==department?item.status == 1?item.type==1?topSeat_move:topSeat_move:topSeat_dis:topSeat_dis:
                      item.toward==2?item.act?leftSeat_act:item.staffDepartment==department?item.status == 1?item.type==1?leftSeat_move:leftSeat_dis:leftSeat_move:leftSeat_dis:
                      item.act?bottomSeat_act:item.staffDepartment==department?item.status == 1?item.type==1?bottomSeat_move:bottomSeat_move:bottomSeat_dis:bottomSeat_dis)+')'
                    }">
                    <span class="textBox noselect">
                        <span class="seatText" v-if="item.type!=2" :style=" {color:item.act?'#fff':item.staffDepartment==department && item.type==1?'#93C8FC':''}">{{item.staffDepartment==department?item.staffName?item.staffName:item.code:item.code}}</span>
                    </span>
                     <el-collapse-transition>
                    <div class="seatPopover" v-show="item.staffDepartment==department&&item.showPopover">
                        <li class="top">
                            <div class="topLeft"> <div class="circle"></div> {{item.code}}</div>
                            <div class="topRight">
                                 <!--分配工位--><img v-if="!item.staffName" title="分配工位" @click="allocationSeatsDia([item])" src="../../assets/img/icon_272@2x.png" alt="">
                                <!--&lt;!&ndash;取消使用&ndash;&gt;<img @click="cancelMobileSeat([item])" v-if="item.type==0&&item.status == 1" src="../../assets/img/icon_265@2x.png" alt="">-->
                                <!--注销工位--><img @click="seatLogout([item.id])" title="注销工位" v-if="item.type==1 && item.status == 1" src="../../assets/img/icon_268@2x.png" alt="">
        ...
        methods: {
            ...
            getCurrentStaff(){
                getCurrentStaff({
                    ...
                    this.department = this.currentStaffInfo.department
                })
            },
            //取消移动工位
