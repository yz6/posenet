        ...
        <div class="contentBox rightTable">
            <el-form label-position="left" label-width="130px" :model="form">
                ...
                <el-form-item label="预约楼层">
                    <el-select disabled @change="getDepartment" v-model="form.floorId" style="width: 200px" placeholder="请选择">
                        <el-option
                            v-for="item in floorOptions"
                            :key="item.value"
                            :label="item.label +' ('+item.availableCount+'个可预约工位)'"
                            :value="item.value">
                        </el-option>
                    </el-select>
                ...
                <el-form-item label=" ">
                    <el-button :disabled="!canChoice"  @click="subMitReserve" type="primary">
...
<script>
    import {getBuilding, getFloor,getDepartment,reservation,getCurrentStaff} from "../api/getData";
    ...
    export default {
        ...
        activated(){
            ...
            this.checkSeatCode = []
            this.checkSeatIds = []
            this.routeParams = this.$route.params.seat

            this.routeParams.forEach((v,i)=>{
                this.checkSeatCode.push(v.code)
                this.checkSeatIds.push(v.id)
            })
            this.firstOptions={
                disabledDate(time) {
                    return time.getTime() <= new Date(new Date() - 24 * 60 * 60 * 1000);
        ...
        methods: {
            ...
            subMitReserve() {
                getCurrentStaff({
                    ...
                    if(3-res.result.reservationSeatCount >= (this.form.type==1?1:this.form.seatNum)){
                        ...
                        var data = {}
                        if(this.form.type==1){
                            if(params.startDate<(new Date()).valueOf()){
                                data={
                                    seatIds:this.checkSeatIds,
                        ...
                        reservation(data).then(res=>{
                            if(res.result){
                                this.$router.push({name:'reserve_success',params:{data:{
                                            ...
                                            startDate:params.startString,
                                            endDate:params.endString,
            ...
            getDepartment(v){
                ...
                getDepartment({
                    ...
                    this.floorLabel(this.form.floorId)
                })
            },
